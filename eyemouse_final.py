...
MIN_BLINK_FRAMES = 3  # 연속 깜빡임 감지를 위한 최소 프레임 수
SINGLE_CLICK_FRAMES = 7  # 일반 클릭을 위한 최소 프레임 수
DOUBLE_BLINK_INTERVAL = 0.8  # 연속 깜빡임으로 인식할 최대 시간 간격 (초)
...
is_clicking = False  # 현재 클릭 유지 상태인지 여부
...
video.set(cv2.CAP_PROP_FRAME_WIDTH, 640)  # 해상도를 640x480으로 낮춤
...
cv2.resizeWindow(WINDOW_NAME, 640, 480)  # 초기 윈도우 크기를 640x480으로 설정
...
last_action_time = 0
last_action_msg = ''
ACTION_MSG_DURATION = 1.0  # 1초간 표시
...
def zoom_eye_region(frame, eye_landmarks, zoom_factor=2.0):
    ...
    # 확대할 영역의 크기 계산 (화면 크기의 1/12로 더 작게)
    zoom_size = min(frame.shape[0], frame.shape[1]) // 12
    ...
    if eye_region.size > 0:
        ...
        # 확대된 영역에 홍채 위치 표시
        cv2.circle(eye_region, (zoomed_center_x, zoomed_center_y), 3, (0, 255, 0), -1)
        
        # 확대된 영역을 원본 프레임에 합성 (화면 크기에 비례하여 위치 조정)
        h, w = eye_region.shape[:2]
        margin = min(frame.shape[0], frame.shape[1]) // 30  # 화면 크기의 1/30
        frame[margin:margin+h, margin:margin+w] = eye_region
        
        # 확대된 영역 표시
        cv2.rectangle(frame, (margin, margin), (margin+w, margin+h), (0, 255, 0), 1)
...
while True:
            ...
            if ear < EAR_THRESHOLD:
                ...
                if is_blinking:  # 깜빡임이 끝났을 때
                    if not is_first_blink:
                        # 첫 번째 깜빡임 감지
                        if blink_counter >= SINGLE_CLICK_FRAMES:
                            # 일반 클릭으로 처리
                            pyautogui.click()
                            print(f"Single click (duration: {blink_counter} frames)")
                            last_action_time = time.time()
                            last_action_msg = 'Clicked!'
                        else:
                            # 연속 깜빡임을 위한 첫 번째 깜빡임으로 처리
                            is_first_blink = True
                            last_blink_time = current_time
                            print("First blink for double blink detected")
                    else:
                        # 두 번째 깜빡임 감지
                        time_diff = current_time - last_blink_time
                        if time_diff <= DOUBLE_BLINK_INTERVAL:
                            # 연속 깜빡임으로 인식 - 클릭 유지/해제 토글
                            if not is_clicking:
                                pyautogui.mouseDown()
                                is_clicking = True
                                print("Click hold started")
                                last_action_time = time.time()
                                last_action_msg = 'Drag Start!'
                            else:
                                pyautogui.mouseUp()
                                is_clicking = False
                                print("Click hold released")
                                last_action_time = time.time()
                                last_action_msg = 'Drag End!'
                        else:
                            # 시간 간격이 너무 길면 아무 동작도 하지 않음
                            print("Blink interval too long - no action")
                        is_first_blink = False
                    is_blinking = False
                blink_counter = 0

            # 첫 번째 깜빡임 후 일정 시간이 지나면 초기화
            if is_first_blink and (current_time - last_blink_time) > BLINK_TIMEOUT:
                print("Blink timeout - resetting")
                is_first_blink = False

            # 눈을 뜨고 있을 때만 커서 위치 업데이트
            if blink_counter == 0:
                ...
                if calibration_complete and calib_box is not None:
                    ...
                    # 클릭 유지 상태일 때는 드래그, 아닐 때는 일반 이동
                    if is_clicking:
                        pyautogui.dragTo(smoothed_mx, smoothed_my)
                    else:
    ...
    # 클릭 상태 표시
    if is_clicking:
        cv2.putText(frame, "Click Hold", (w - 150, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)

    # 클릭/드래그 메시지 표시
    if last_action_msg and (time.time() - last_action_time) < ACTION_MSG_DURATION:
        cv2.putText(frame, last_action_msg, (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 3)
